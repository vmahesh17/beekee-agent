...
public class BeeKee_AgentApp_TestSuite2 extends Actions_Android {
	...
	public static void BeeKee_AgentApp_SignIn_Cancel_EditUsername() {
		try {
			System.out.println("Started Exection for TC > " + new Exception().getStackTrace()[0].getMethodName());
			BeeKee_SignInPage.edit_Cancel_UserName(prop.getProperty("agent.newuser"));

		} catch (Exception e) {
			System.err.println("Failed Exection for TC > " + new Exception().getStackTrace()[0].getMethodName());
			ATUReports.add("Sign", "Unable to perform SignIn ", "", "", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
	...
	public static void BeeKee_AgentApp_SignIn_ShowHidePassword() {
		try {
			...
			clearAndType(TXT_PASSWORD, prop.getProperty("agent.password"));
			...
			click(BTN_EYEICON);
			Assert.assertEquals(getAttribute(BTN_EYEICON, "aria-label").equalsIgnoreCase("eye"), true);
	...
	@Test(enabled = true, priority = 9, groups = { "SignIn", "P1" })
	public static void BeeKee_AgentApp_SignIn_Save_Username_Password() {

		try {
			System.out.println("Started Exection for TC > " + new Exception().getStackTrace()[0].getMethodName());
			BeeKee_SignInPage.signInToApplication(true, prop.getProperty("agent.newuser").trim(),
	...
	/// NEW CASES 12/04 -- SIGNUP

	@Test(priority = 10, enabled = true, groups = {"SignUp", "P1" })
	...
	@Test(priority = 11, enabled = true, groups = {"SignUp", "P1" })
	...
	@Test(priority = 12, enabled = false, groups = { "P2" })
	public static void BeeKee_AgentApp_ForgotPassword() throws Exception {
		try {
			System.out.println("Started Exection for TC > " + new Exception().getStackTrace()[0].getMethodName());
			BeeKee_ForgotPasswordPage.verifyForgotPassword("","","","");
		} catch (Exception e) {
			System.err.println("Failed Exection for TC > " + new Exception().getStackTrace()[0].getMethodName());
			ATUReports.add("Forgot Password", "Unable to do forgot password", "", "", LogAs.FAILED,
					new CaptureScreen(ScreenshotOf.DESKTOP));
		}
	}
